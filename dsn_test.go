...
func TestIPv6(t *testing.T) {
	dsn, err := ParseOsloDBConnectionString("mysql://user:pass@[::1]:3306/nova")
	require.NoError(t, err)
	require.Equal(t, "user:pass@tcp([::1]:3306)/nova?parseTime=true", dsn)
}
func TestMalformedURLDoesNotExposeCredentials(t *testing.T) {
	_, err := ParseOsloDBConnectionString("mysql://user:private%zz@localhost/db")
	require.Error(t, err)
