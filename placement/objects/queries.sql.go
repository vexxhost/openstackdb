...
const GetResourceProviderInventories = `-- name: GetResourceProviderInventories :many
SELECT 
    rp.name as hostname,
    rc.name as resource_type,
    i.total,
...
type GetResourceProviderInventoriesRow struct {
...
func (q *Queries) GetResourceProviderInventories(ctx context.Context) ([]GetResourceProviderInventoriesRow, error) {
	rows, err := q.db.QueryContext(ctx, GetResourceProviderInventories)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetResourceProviderInventoriesRow
	for rows.Next() {
		var i GetResourceProviderInventoriesRow
		if err := rows.Scan(
			&i.Hostname,
			&i.ResourceType,
			&i.Total,
