...
func ParseOsloDBConnectionString(connectionString string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid database connection URL")
	}
	...
	var dsn string
	if userPart != "" {
		dsn = fmt.Sprintf("%s@tcp(%s)/%s", userPart, net.JoinHostPort(host, port), database)
	} else {
		dsn = fmt.Sprintf("tcp(%s)/%s", net.JoinHostPort(host, port), database)
	}
